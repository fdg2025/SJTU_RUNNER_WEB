...
def test_correct_captcha_url():
    ...
    # 使用修正后的URL格式
    captcha_url = f'https://pe.sjtu.edu.cn/jaccount/captcha?uuid={captcha_uuid}&t={timestamp}'
    ...
    try:
        ...
        if status == 200 and len(image_data) > 0:
            ...
            if image_data.startswith(b'\xff\xd8\xff') or image_data.startswith(b'\x89PNG'):
                ...
                # 保存验证码图片
                filename = f'corrected_captcha_test.jpg'
...
def test_login_submission():
    ...
    # 获取验证码信息
    captcha_info = test_correct_captcha_url()
    ...
    try:
        status, headers, body, new_cookies = make_request(
            'https://pe.sjtu.edu.cn/jaccount/login',
            method='POST',
            data=login_data,
...
def main():
    ...
    if captcha_info and captcha_info['success']:
        ...
        print('\n💡 实现总结:')
        print('1. ✅ 访问SJTU登录页面获取JSESSIONID')
        print('2. ✅ 从页面提取验证码UUID')
        print('3. ✅ 使用正确的URL格式获取验证码图片')
